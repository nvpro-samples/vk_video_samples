...
-------------
  ...
  benchmark_results.json      - Machine-readable JSON results
...
---------------
  ...
  python3 aq_quality_benchmark.py \\
      ...
      --rate-control-mode vbr --average-bitrate 2000 \\
      --output-dir /tmp/benchmark
...
def calculate_psnr(
    ...
    """Calculate PSNR using ffmpeg. Returns (y, u, v, avg, min, max)."""
...
def calculate_vmaf(
    ...
    """Calculate VMAF using ffmpeg. Returns VMAF score."""
...
def decode_to_yuv(
    ...
    """Decode an encoded file to raw YUV using ffmpeg."""
    ...
    try:
        proc = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
        return proc.returncode == 0 and output_yuv.exists()
    except Exception as e:
        if verbose:
...
def calculate_quality_metrics(
    ...
    if not decode_to_yuv(result.output_file, decoded_yuv, verbose):
    ...
    psnr = calculate_psnr(decoded_yuv, reference_yuv, width, height, verbose)
    ...
    result.vmaf_score = calculate_vmaf(decoded_yuv, reference_yuv, width, height, verbose)
...
For more information, see README-AQ.md in the vulkan-video-samples repository.
    ...
    rate_ctrl.add_argument("--average-bitrate", type=int, 
                           help="Target average bitrate in kilobits per second (e.g., 2000 for 2 Mbps)")
    ...
    for i, config in enumerate(aq_configs, 1):
        ...
        if result.encode_success:
            ...
            if not args.skip_psnr or not args.skip_vmaf:
                ...
                calculate_quality_metrics(result, reference_yuv, args.width, args.height, work_dir, args.verbose)

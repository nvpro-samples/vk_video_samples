                                                 ...
                                                 VkSharedBaseObj<VulkanDeviceMemoryImpl>& vulkanDeviceMemory)
{
    ...
    if (result != VK_SUCCESS) {
        vv_assert(!"Create Buffer failed!");
        return result;
    }
    ...
    if (result != VK_SUCCESS) {
        vkDevCtx->DestroyBuffer(*vkDevCtx, buffer, nullptr);
        vv_assert(!"Bind buffer memory failed!");
        return result;
    }
                                               ...
                                               VkDeviceSize initializeBufferMemorySize)
{
    ...
    if (result != VK_SUCCESS) {
        vv_assert(!"Create new buffer failed!");
        return result;
    }
...
VkDeviceSize VulkanBitstreamBufferImpl::Resize(VkDeviceSize newSize, VkDeviceSize copySize, VkDeviceSize copyOffset)
{
    ...
    if (copySize) {
        ...
        vv_assert(pInitializeBufferMemory);
        vv_assert(copySize <= maxSize);
    }
    VkResult result = CreateBuffer(m_vkDevCtx,
                                   m_queueFamilyIndex,
                                   m_usage,
...
uint8_t* VulkanBitstreamBufferImpl::CheckAccess(VkDeviceSize offset, VkDeviceSize size) const
{
    if (offset + size <= m_bufferSize) {
        ...
        if (bufferDataPtr == nullptr) {
            vv_assert(!"Bad buffer access - can't map buffer!");
            return nullptr;
        }
        return bufferDataPtr + offset;
    }

    vv_assert(!"Bad buffer access - out of range!");
    return nullptr;
}
                                                    ...
                                                    VkDeviceSize srcOffset, VkDeviceSize size) const
{
    ...
    if (readData == nullptr) {
        vv_assert(!"Could not CopyDataToBuffer!");
        return -1;
    }
                                                       ...
                                                       VkDeviceSize srcOffset, VkDeviceSize dstOffset, VkDeviceSize size)
{
    ...
    if (readData == nullptr) {
        vv_assert(!"Could not CopyDataFromBuffer!");
        return -1;
    }
...
uint8_t* VulkanBitstreamBufferImpl::GetDataPtr(VkDeviceSize offset, VkDeviceSize &maxSize)
{
    ...
    if (readData == nullptr) {
        vv_assert(!"Could not GetDataPtr()!");
        return nullptr;
    }
...
const uint8_t* VulkanBitstreamBufferImpl::GetReadOnlyDataPtr(VkDeviceSize offset, VkDeviceSize &maxSize) const
{
    uint8_t* readData = CheckAccess(offset, 1);
    if (readData == nullptr) {
        vv_assert(!"Could not GetReadOnlyDataPtr()!");
        return nullptr;
    }
    maxSize = m_bufferSize - offset;
    return readData;
...
{
    vv_assert(index < (uint32_t)m_streamMarkers.size());

                               ...
                               VkSharedBaseObj<VulkanDeviceMemoryImpl>& vulkanDeviceMemory)
{
    ...
    if (!vkDeviceMemory) {
        vv_assert(!"Couldn't allocate host memory!");
...
{
    vv_assert(memoryRequirements.size ==
    ...
    if (result != VK_SUCCESS) {
        vv_assert(!"Couldn't allocate device memory!");
        return result;
    }
                                            ...
                                            bool clearMemory)
{
    ...
    if (result != VK_SUCCESS) {
        vv_assert(!"Couldn't CreateDeviceMemory()!");
        return result;
    }
                                                  ...
                                                  VkDeviceSize memoryOffset) const
{
    if ((pData == nullptr) || (size == 0)) {
        vv_assert(!"Couldn't CopyDataToMemory()!");
        return VK_ERROR_INITIALIZATION_FAILED;
    }

    uint8_t* pDst = NULL;
    vv_assert((memoryOffset + size) <= m_memoryRequirements.size);
    ...
    if (result != VK_SUCCESS) {
        vv_assert(!"Couldn't FlushMappedMemoryRange()!");
        return result;
    }
...
uint8_t* VulkanDeviceMemoryImpl::CheckAccess(VkDeviceSize offset, VkDeviceSize size)
{
    if (size == VK_WHOLE_SIZE) {
        ...
        if (offset < size) {
            ...
            // invalid offset
            vv_assert(!"CheckAccess() failed - buffer out of range!");
            return nullptr;
        }
    }
...
int64_t VulkanDeviceMemoryImpl::MemsetData(uint32_t value, VkDeviceSize offset, VkDeviceSize size)
{
    ...
    if (setData == nullptr) {
        vv_assert(!"MemsetData() failed - buffer out of range!");
        return -1;
    }

    vv_assert(size < std::numeric_limits<size_t>::max());
                                                 ...
                                                 VkDeviceSize srcOffset, VkDeviceSize size)
{
    ...
    if (readData == nullptr) {
        vv_assert(!"CopyDataToBuffer() failed - buffer out of range!");
        return -1;
    }
    vv_assert(size < std::numeric_limits<size_t>::max());
                                                    ...
                                                    VkDeviceSize dstOffset, VkDeviceSize size)
{
    ...
    if (writeData == nullptr) {
        vv_assert(!"CopyDataFromBuffer() failed - buffer out of range!");
        return -1;
    }
                                                   ...
                                                   VkDeviceSize srcOffset, VkDeviceSize dstOffset, VkDeviceSize size)
{
    ...
    if ((srcPtr == nullptr) || (maxSize < size)) {
        vv_assert(!"GetReadOnlyDataPtr() failed - buffer out of range!");
        return -1;
    }
...
uint8_t* VulkanDeviceMemoryImpl::GetDataPtr(VkDeviceSize offset, VkDeviceSize &maxSize)
{
    ...
    if (readData == nullptr) {
        vv_assert(!"GetDataPtr() failed - buffer out of range!");
        return nullptr;
    }

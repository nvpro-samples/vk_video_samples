...
struct EncodingFrame {
    ...
    VkFence frameCompleteFence; // If valid, the fence is signaled when the encoder is done decoding the frame.
    ...
    VkSemaphore frameCompleteSemaphore; // If valid, the semaphore is signaled when the encoder is done decoding the frame.
...
class VulkanVideoEncodeFrameBuffer : public VkVideoRefCountBase {
public:
    // Synchronization
    ...
    virtual int32_t DequeueEncodedPicture(EncodingFrame* pEncodedFrame) = 0;
    virtual int32_t ReleaseDisplayedPicture(EncodedFrameRelease** pEncodedFramesRelease, uint32_t numFramesToRelease) = 0;
    ...
    virtual int32_t GetCurrentImageResourceByIndex(int8_t referenceSlotIndex,
                                                   ...
                                                   VkImageLayout newInputImageLayerLayout = VK_IMAGE_LAYOUT_MAX_ENUM) = 0;
    virtual int32_t GetCurrentImageResourceByIndex(int8_t referenceSlotIndex,
                                                   VkSharedBaseObj<VkImageResourceView>& dpbImageView,
                                                   VkSharedBaseObj<VkImageResourceView>& inputImageView) = 0;
    virtual int32_t ReleaseImageResources(uint32_t numResources, const uint32_t* indexes) = 0;
    virtual uint64_t SetPicNumInEncodeOrder(int32_t picId, uint64_t picNumInEncodeOrder) = 0;
    virtual int32_t SetPicNumInDisplayOrder(int32_t picId, int32_t picNumInDisplayOrder) = 0;
    virtual size_t GetSize() = 0;

    virtual ~VulkanVideoEncodeFrameBuffer() { }

...
#include "encoder_argv_completion.h"
...
public:
    ...
    virtual VkResult GetBitstream() {
        // Was: join, discard the verdict, answer VK_SUCCESS. A caller had no
        // way to learn that the threads had not completed, or that the
        // buffered bitstream never reached the file, and the quality harness
        // read the zero exit that followed as a pass.
        //
        // Wait, then check the output, then let the caller release the owner.
        // Cached, so a repeated call neither joins nor flushes twice and never
        // turns a recorded failure into a success.
        return m_completion.Complete(
            (m_encoderConfig != nullptr) && !m_encoderConfig->disableFileOutput,
            [this]() -> bool {
                return m_encoder ? m_encoder->WaitForThreadsToComplete() : false;
            },
            [this]() -> bool {
                return ArgvCompletionState::FlushFileOutput(
...
private:
    ...
    // The completion verdict, cached so repeated calls agree.
    ArgvCompletionState              m_completion;
};

 ...
 * colour model -- and accepts thirteen pairs over twelve distinct enumerants:
 * four ENCODABLE_DIRECT (NV12, P010, NV24, S410), six ENCODABLE_VIA_FILTER
 * Y'CbCr (P012, I420 8/10/12-bit, and the packed 4:4:4 layouts AYUV and Y410
 * declared VK_VIDEO_ENCODER_COLOR_MODEL_YCBCR) and three ENCODABLE_VIA_FILTER
 * RGB (R8G8B8A8_UNORM, B8G8R8A8_UNORM, A8B8G8R8_UNORM_PACK32). The pair count
 * exceeds the enumerant count because R8G8B8A8_UNORM carries two accepted
 * readings, AYUV and RGBA, and only the declared colour model separates them.
 * "Claims to support" is a statement about that table. Whether a format
 * REGISTERS, and onto WHICH INPUT PATH, is a statement about a device, a
 * session and a descriptor -- three things the table does not see.
 *
 ...
 * The packed Y'CbCr readings are NOT walked here. They are reachable only by
 * declaring a colour model, and this matrix varies the format alone.
 *
...
enum Group { G_8BIT = 0, G_10BIT = 1, G_12BIT = 2, G_NONE = 3 };
...
const Row kRows[] = {
    ...
    {"P012  G12X4_B12X4R12X4_2PLANE_420...",
     VK_FORMAT_G12X4_B12X4R12X4_2PLANE_420_UNORM_3PACK16, ARM_DIRECT,
     G_12BIT, H265},
...
RowResult RunRow(const Row& row, bool verbose)
{
    ...
    // The filter is REQUESTED for every row, including the DIRECT ones. A
    // session that does not ask for it cannot route FILTER at all, and a
    // matrix in which the RGBA rows alone had it would not be one experiment.
    config.enablePreprocessFilter = VK_TRUE;
    ...
    if (row.arm != ARM_CONTROL) {
        ...
        if (CreateImage(fns, phys, device, row.format, w.tiling, w.usage,
                        w.flags, w.profileList, row.codec, depth, &wimg)) {
...
int main(int argc, char** argv)
{
    ...
    bool groupHasDevice[G_NONE + 1] = {false, false, false, true};
    ...
    for (size_t i = 0; i < kNumRows; i++) {
        ...
        const VkEncInputFormatClass cls = VkEncClassifyInputFormat(row.format);
        ...
        std::printf("      taxonomy=%s  planeCount=%u  isRgba=%u\n",
                    ...
                    (unsigned)VkEncIsRgbaInputFormat(row.format));
        ...
        switch (row.arm) {
            ...
            case ARM_FILTER_YCBCR:
                Check(row.name,
                      cls == VK_ENC_INPUT_FORMAT_ENCODABLE_VIA_FILTER,

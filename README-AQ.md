...
| `--average-bitrate <kbps>` | encoder default | Target bitrate in kilobits per second |
...
```bash
for bitrate in 1000 2000 5000 10000; do
    python3 scripts/aq_quality_benchmark.py \
        --input video.yuv --width 1920 --height 1080 \
        --codec hevc --num-frames 100 \
        --average-bitrate $bitrate \
        --output-dir /tmp/benchmark_${bitrate}kbps \
        --vulkan-samples-root /path/to/vulkan-video-samples
done
```

#### AV1 Benchmark with Custom AQ Strength
...
If you see "VMAF calculation failed", install FFmpeg with libvmaf:

```bash
# Ubuntu/Debian
sudo apt-get install ffmpeg

# Or build FFmpeg with --enable-libvmaf
```
